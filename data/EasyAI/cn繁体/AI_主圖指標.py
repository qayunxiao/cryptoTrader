...
indicator(title="AI_主圖指標", shorttitle="AI_主圖指標",overlay=true,max_boxes_count = 300,max_labels_count=500,max_lines_count=500,max_bars_back=500)
//買賣點

// HighLow_Indicators=input(false,title ="AI_天地")
Channel_Indicators=input(true,title ="通道")
Chip_Indicators=input(true,title ="籌碼峰")
BuySell_Indicators=input(true,title ="買賣提示")
// TD9 = input(false,title ="神奇九轉")
RGTrend_Indicators=input(true,title ="紅綠趨勢線")
UpDownTrend_Indicators=input(true,title ="支撐壓力線")
//Double_Trend_Indicators=input(false,title ="雙均線戰法")
// RSI_Indicators=input(false,title ="多空提示預警")
DirectionLine_Indicators=input(true,title ="方向線與多空背景")
//OBV_auxiliary_Indicators=input(false,title ="副圖OBV輔助DEMA")
...
group2 = "AI_通道延長"
showPearsonInput = input.bool(false, "皮爾森R", group = group2)
extendLeftInput = input.bool(false, "延長左側", group = group2)
extendRightInput = input.bool(true, "延長右側", group = group2)
extendStyle = switch
...
plotshape(buySignal and showsignals ? up : na, title="買", text="買", location=location.absolute, style=shape.labelup, size=size.tiny,color=color.new(color.green,0), textcolor=color.white)
dnPlot = plot(trend == 1 ? na : dn, title="Down Trend", style=plot.style_linebr, linewidth=2, color=color.red)
sellSignal = trend == -1 and trend[1] == 1
plotshape(sellSignal ? dn : na, title="DownTrend Begins", location=location.absolute, style=shape.circle, size=size.tiny, color=color.new(color.red,0))
plotshape(sellSignal and showsignals ? dn : na, title="賣", text="賣", location=location.absolute, style=shape.labeldown, size=size.tiny, color=color.new(color.red,0), textcolor=color.white)
...
alertcondition(buySignal, title="買點提示", message="買點提示")
alertcondition(sellSignal, title="賣點提示", message="賣點提示")
//..................................................................AI_買賣提示結束
...
alertcondition(sell, title='支撐買點', message='買點提示')
alertcondition(buy, title='壓力賣點', message='賣點提示')
...
//-----------------------------------------------OBV_auxiliary_Indicators 輸出dema 21 開始
...
//-----------------------------------------------OBV_auxiliary_Indicators 輸出dema 21 結束
...
// plotshape(longCondition_rsi, title = "Buy Signal", text ="開多", textcolor = color.white, style=shape.labelup, size = size.normal, location=location.belowbar, color =color.new(color.blue, 10))
// plotshape(shortCondition_rsi, title = "Sell Signal",text ="開空", textcolor = color.white, style=shape.labeldown, size = size.normal, location=location.abovebar, color =color.new(color.red, 10))

// alertcondition(longCondition_rsi, title="開多", message = "開多")
// alertcondition(shortCondition_rsi, title="開空", message = "開空")
//-----------------------------------------------RSI_Indicators多空結束
